...
def validate_dataframe_values(df):
    ...
    for column, valid_values in allowed_values.items():
        ...
        if valid_values == 'numeric':
            non_numeric = df[~df[column].isna() & (~df[column].astype(str).str.replace('.', '').str.isnumeric())]
    ...
    try:
        ...
        if results:
            ...
            for column, invalid_values in results.items():
                ...
                if column in df.columns and invalid_values != ["Column not found in dataframe"]:
...
def impute_gcs_columns(df):
    ...
    medians = {col: df[col].median(skipna=True) for col in ['gcs_eye', 'gcs_verbal', 'gcs_motor']}
...
def impute_group_data(df, main_col, subcols, default_value=92, main_default=0):
    ...
    df.loc[df[main_col].isna() & df[subcols].eq(default_value).all(axis=1), main_col] = main_default
    return df
...
def clean_data():
    ...
    group_imputations = {
        'vomiting_reported': ['vomiting_episode_count', 'vomiting_onset_time', 'last_vomiting_time'],
        'headache_reported': ['headache_intensity', 'headache_onset_time'],
        'altered_mental_status': ['ams_agitated', 'ams_sleepy', 'ams_slow_response', 'ams_repetitive_questions',
                                  'ams_other'],
        'palpable_skull_fracture': ['palpable_skull_fracture_depressed'],
        'basilar_skull_fracture_signs': ['basilar_skull_fracture_hemotympanum', 'basilar_skull_fracture_otorrhea',
        ...
        'trauma_above_clavicles': ['trauma_face', 'trauma_neck', 'trauma_scalp_frontal', 'trauma_scalp_occipital',
                                   'trauma_scalp_parietal', 'trauma_scalp_temporal'],
        'neurological_deficit_present': ['neurological_deficit_motor', 'neurological_deficit_sensory',
                                         'neurological_deficit_cranial_nerve', 'neurological_deficit_reflexes',
                                         'neurological_deficit_other'],
        'other_substantial_injury': ['other_injury_extremity', 'other_injury_laceration', 'other_injury_cspine',
                                     'other_injury_chest_back_flank', 'other_injury_abdomen', 'other_injury_pelvis',
                                     'other_injury_other'],
        'sedation_for_ct': ['sedation_for_ct_agitated', 'sedation_for_ct_age', 'sedation_for_ct_tech_request',
                            'sedation_for_ct_other'],
        'head_ct_planned': ['ct_reason_age', 'ct_reason_amnesia', 'ct_reason_ams', 'ct_reason_clinical_skull_fracture',
                            'ct_reason_headache', 'ct_reason_scalp_hematoma', 'ct_reason_loss_of_consciousness',
                            'ct_reason_mechanism', 'ct_reason_neurological_deficit', 'ct_reason_request_md',
                            'ct_reason_request_parent', 'ct_reason_request_trauma', 'ct_reason_seizure',
                            'ct_reason_vomiting', 'ct_reason_xray_skull_fracture', 'ct_reason_other']
    }
