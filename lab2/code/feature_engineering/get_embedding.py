...
def get_embedding(config_path, images_long, patches, filepaths):
    config = yaml.safe_load(open(config_path, "r"))

    ckpt_files = [f for f in os.listdir("checkpoints") if f.endswith('.ckpt')]
    checkpoint_file = max(ckpt_files, key=lambda f: os.path.getmtime(os.path.join("checkpoints", f)))
    ...
    #images_long, patches, filepaths = make_data(patch_size=config["data"]["patch_size"])
    ...
    for i in tqdm(range(len(images_long))):
        embedding_df = pd.DataFrame(embeddings[i], columns=[f"ae{i}" for i in range(config["autoencoder"]["embedding_size"])])
        ...
        # save to csv
        print(f"../../data/transfer_data/{filepaths[i][-11:-4]}_ae.csv")
        embedding_df.to_csv(f"../../data/transfer_data/{filepaths[i][-11:-4]}_ae.csv", index=False)
